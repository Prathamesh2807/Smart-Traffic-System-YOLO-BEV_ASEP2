...
# ─────────────────────────────────────────────────────────
#  Road asphalt texture (generated once, cached)
# ─────────────────────────────────────────────────────────
_ROAD_TEXTURE: pygame.Surface | None = None


def _generate_road_texture():
    """One-time translucent surface with scattered noise dots for asphalt grain."""
    global _ROAD_TEXTURE
    if _ROAD_TEXTURE is not None:
        return
    _ROAD_TEXTURE = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.SRCALPHA)
    rng = random.Random(ROAD_TEXTURE_SEED)
    half = ROAD_WIDTH // 2
    total_area = ROAD_WIDTH * WINDOW_HEIGHT + WINDOW_WIDTH * ROAD_WIDTH
    n_dots = int(total_area * ROAD_TEXTURE_DENSITY)

    for _ in range(n_dots):
        if rng.random() < 0.5:
            x = CENTER_X - half + rng.randint(0, ROAD_WIDTH - 1)
            y = rng.randint(0, WINDOW_HEIGHT - 1)
        else:
            x = rng.randint(0, WINDOW_WIDTH - 1)
            y = CENTER_Y - half + rng.randint(0, ROAD_WIDTH - 1)
        x = max(0, min(WINDOW_WIDTH - 1, x))
        y = max(0, min(WINDOW_HEIGHT - 1, y))
        brightness = rng.randint(38, 82)
        alpha = rng.randint(15, 40)
        _ROAD_TEXTURE.set_at((x, y), (brightness, brightness, brightness, alpha))
...
def draw_scene(surface: pygame.Surface, fonts: dict):
    ...
    if _ROAD_TEXTURE is not None:
        surface.blit(_ROAD_TEXTURE, (0, 0))

    # Subtle road edge highlights (inner curb glow lines)
    hl = COLOR_ROAD_HIGHLIGHT
    pygame.draw.line(surface, hl, (CENTER_X - half + 1, 0),
                     (CENTER_X - half + 1, WINDOW_HEIGHT), 1)
    pygame.draw.line(surface, hl, (CENTER_X + half - 1, 0),
                     (CENTER_X + half - 1, WINDOW_HEIGHT), 1)
    pygame.draw.line(surface, hl, (0, CENTER_Y - half + 1),
                     (WINDOW_WIDTH, CENTER_Y - half + 1), 1)
    pygame.draw.line(surface, hl, (0, CENTER_Y + half - 1),
                     (WINDOW_WIDTH, CENTER_Y + half - 1), 1)
...
def _draw_lane_markings(surface: pygame.Surface):
    ...
    while y < CENTER_Y - half:
        ...
        if idx % 2 == 0:
            pygame.draw.circle(surface, refl, (CENTER_X, y + dash // 2), 2)
        idx += 1
        y += dash + gap
    y = CENTER_Y + half
    ...
    while x < CENTER_X - half:
        ...
        if idx % 2 == 0:
            pygame.draw.circle(surface, refl, (x + dash // 2, CENTER_Y), 2)
        idx += 1
        x += dash + gap
    x = CENTER_X + half
...
def draw_signals(surface: pygame.Surface, controller):
    ...
    for direction, (px, py) in _POLE_POSITIONS.items():
        ...
        for name, ly, lit_color in lights:
            ...
            if is_on:
                # Base glow (slightly larger than before)
                half_base = SIGNAL_GLOW_BASE // 2
                glow = _get_glow(lit_color, SIGNAL_GLOW_BASE, 60)
                surface.blit(glow, (light_x - half_base, ly - half_base))
                # Pulsing outer glow — breathes via sine wave
                pulse = (math.sin(now * SIGNAL_PULSE_SPEED) + 1.0) / 2.0
                outer_alpha = 15 + int(pulse * 30)
                outer_alpha = (outer_alpha // 5) * 5   # quantize for cache
                half_outer = SIGNAL_GLOW_OUTER // 2
                glow2 = _get_glow(lit_color, SIGNAL_GLOW_OUTER, outer_alpha)
                surface.blit(glow2, (light_x - half_outer, ly - half_outer))
...
def draw_dashboard(surface: pygame.Surface, fonts: dict,
    ...
    # Translucent panel with gradient header + glassmorphism
    panel = pygame.Surface((DASH_W, DASH_H), pygame.SRCALPHA)
    panel.fill((16, 20, 30, 220))
    # Gradient header glow (top 35 px)
    for i in range(35):
        alpha = int(50 * (1.0 - i / 35.0))
        pygame.draw.line(panel, (50, 90, 170, alpha), (3, i), (DASH_W - 3, i))
    # Inner highlight border (glassmorphism edge)
    pygame.draw.rect(panel, (70, 110, 180, 30),
                     (1, 1, DASH_W - 2, DASH_H - 2), 1, border_radius=12)
    # Outer border
    pygame.draw.rect(panel, (*COLOR_DASH_BORDER, 210),
                     (0, 0, DASH_W, DASH_H), 2, border_radius=12)
    # Bottom accent line
    pygame.draw.line(panel, (50, 90, 170, 80),
                     (6, DASH_H - 3), (DASH_W - 6, DASH_H - 3), 1)

...
def _vehicles_ahead(v: Vehicle, all_vehicles: list) -> list:
    ...
    candidates = [
        ...
        if (u is not v
            ...
            and not u.passed
            # Only filter diverted vehicles that have LEFT the lane (phase >= 1).
            # Phase 0 vehicles are still in the original lane approaching waypoint.
            and not (getattr(u, 'is_diverted', False)
                     and getattr(u, '_divert_phase', 0) >= 1)
...
class Simulation:
    def __init__(self):
        ...
        # Wait time tracking (per direction, rolling buffer)
        self._wait_times: dict[str, list[float]] = {d: [] for d in DIRECTIONS}
    ...
    def _update(self, dt: float):
        ...
        # 3. Count passed vehicles + record wait times
        for v in self.vehicles:
            if v.passed and not hasattr(v, "_counted"):
                v._counted           = True
                self.total_passed   += 1
                self.controller.total_passed = self.total_passed
                # Record waiting time for this vehicle's lane
                self._wait_times[v.direction].append(
                    getattr(v, '_total_wait_time', 0.0))
                # Keep only last 100 entries per direction
                if len(self._wait_times[v.direction]) > 100:
                    self._wait_times[v.direction] = self._wait_times[v.direction][-100:]
        ...
        if self.controller.emergency_active and not self._prev_emergency:
            ...
            log_ambulance_event("[OK] Emergency cleared - normal cycle resumed")
    ...
    def _push_stats(self, queues: dict):
        ...
        # Compute average wait times (last 50 per direction)
        avg_wait = {}
        for d in DIRECTIONS:
            times = self._wait_times[d][-50:]
            avg_wait[d] = round(sum(times) / len(times), 1) if times else 0.0
        all_times = [t for times in self._wait_times.values() for t in times[-50:]]
        overall_avg = round(sum(all_times) / len(all_times), 1) if all_times else 0.0

        # Green direction label (show paired directions)
        gd = self.controller.green_dir
        if gd and not self.controller.emergency_active:
            opp = OPPOSITE.get(gd, "")
            green_label = f"{gd.capitalize()} + {opp.capitalize()}"
        elif gd:
            green_label = gd.capitalize()
        else:
            green_label = None

        stats = {
            "mode":            self.controller.current_mode,
            "green_dir":       green_label,

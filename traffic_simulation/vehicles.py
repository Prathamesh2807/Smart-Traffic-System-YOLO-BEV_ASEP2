...
from config import (
    ...
    SHADOW_OFFSET_X, SHADOW_OFFSET_Y, SHADOW_ALPHA,
    HEADLIGHT_BEAM_LEN, HEADLIGHT_BEAM_SPREAD, HEADLIGHT_BEAM_ALPHA,
    BRAKE_GLOW_RADIUS, BRAKE_GLOW_ALPHA,
    TURN_WEIGHTS, TURN_EXIT_MAP, OPPOSITE,
)
...
# ─────────────────────────────────────────────────────────
#  Visual effect caches (shadows, headlights, brake glow)
# ─────────────────────────────────────────────────────────
_SHADOW_CACHE: dict[tuple, pygame.Surface] = {}
_HEADLIGHT_CACHE: dict[str, tuple] = {}
_BRAKE_GLOW: pygame.Surface | None = None


def _get_shadow(vtype: str, direction: str) -> pygame.Surface:
    """Cached semi-transparent ground shadow ellipse per vehicle type."""
    key = (vtype, direction)
    if key not in _SHADOW_CACHE:
        props = VEHICLE_TYPES[vtype]
        vw, vh = props["width"], props["height"]
        if direction in ("north", "south"):
            sw, sh = vw + 8, max(8, vh // 3)
        else:
            sw, sh = vh + 8, max(8, vw // 3)
        surf = pygame.Surface((sw, sh), pygame.SRCALPHA)
        pygame.draw.ellipse(surf, (0, 0, 0, SHADOW_ALPHA), (0, 0, sw, sh))
        _SHADOW_CACHE[key] = surf
    return _SHADOW_CACHE[key]


def _get_headlight(direction: str) -> tuple:
    """Cached translucent headlight beam cone. Returns (surface, offset_from_nose)."""
    if direction in _HEADLIGHT_CACHE:
        return _HEADLIGHT_CACHE[direction]

    bl = HEADLIGHT_BEAM_LEN
    bs = HEADLIGHT_BEAM_SPREAD
    nw = 4  # half-width at nose end

    if direction in ("north", "south"):
        w, h = bs * 2 + 2, bl
        cx = w // 2
        if direction == "north":  # vehicle moves south (down), beam points down
            outer = [(cx - nw, 0), (cx + nw, 0), (cx + bs, h), (cx - bs, h)]
            inner = [(cx - nw, 0), (cx + nw, 0),
                     (cx + bs // 2, h * 2 // 3), (cx - bs // 2, h * 2 // 3)]
            offset = (-cx, 0)
        else:  # south: vehicle moves north (up), beam points up
            outer = [(cx - nw, h), (cx + nw, h), (cx + bs, 0), (cx - bs, 0)]
            inner = [(cx - nw, h), (cx + nw, h),
                     (cx + bs // 2, h // 3), (cx - bs // 2, h // 3)]
            offset = (-cx, -h)
    else:
        w, h = bl, bs * 2 + 2
        cy = h // 2
        if direction == "west":  # vehicle moves east (right), beam points right
            outer = [(0, cy - nw), (0, cy + nw), (w, cy + bs), (w, cy - bs)]
            inner = [(0, cy - nw), (0, cy + nw),
                     (w * 2 // 3, cy + bs // 2), (w * 2 // 3, cy - bs // 2)]
            offset = (0, -cy)
        else:  # east: vehicle moves west (left), beam points left
            outer = [(w, cy - nw), (w, cy + nw), (0, cy + bs), (0, cy - bs)]
            inner = [(w, cy - nw), (w, cy + nw),
                     (w // 3, cy + bs // 2), (w // 3, cy - bs // 2)]
            offset = (-w, -cy)

    surf = pygame.Surface((w, h), pygame.SRCALPHA)
    pygame.draw.polygon(surf, (255, 250, 200, HEADLIGHT_BEAM_ALPHA), outer)
    pygame.draw.polygon(surf, (255, 255, 220, min(255, HEADLIGHT_BEAM_ALPHA + 12)), inner)
    _HEADLIGHT_CACHE[direction] = (surf, offset)
    return (surf, offset)


def _get_brake_glow() -> pygame.Surface:
    """Cached red brake light glow circle."""
    global _BRAKE_GLOW
    if _BRAKE_GLOW is None:
        r = BRAKE_GLOW_RADIUS
        _BRAKE_GLOW = pygame.Surface((r * 2, r * 2), pygame.SRCALPHA)
        pygame.draw.circle(_BRAKE_GLOW, (255, 30, 30, BRAKE_GLOW_ALPHA), (r, r), r)
        pygame.draw.circle(_BRAKE_GLOW,
                           (255, 80, 60, min(255, BRAKE_GLOW_ALPHA + 30)), (r, r), r // 2)
    return _BRAKE_GLOW
...
# ─────────────────────────────────────────────────────────
#  Intersection turn geometry  (90° and 180° arcs)
#
#  Each arc starts on the INCOMING lane centre and ends
#  precisely on the correct OUTGOING lane centre so the
#  vehicle exits on the right side of the road.
#
#  Lane layout (standard left-half / right-half split):
#    Vertical:   left (x=lxN) = southbound,  right (x=lxS) = northbound
#    Horizontal: top  (y=lyW) = eastbound,   bottom(y=lyE) = westbound
# ─────────────────────────────────────────────────────────

def _setup_turn_path(v, approach: str, exit_to: str):
    """
    Configure a 3-phase waypoint turn for a vehicle at the intersection.
    Arcs are calculated so exit positions align with outgoing lane centres.
    """
    R   = ROAD_WIDTH // 2    # 75 px
    LW  = LANE_WIDTH         # 75 px
    ...
    # Lane centres
    lxN = CX - LW // 2      # 603 — north incoming / south outgoing
    lxS = CX + LW // 2      # 677 — south incoming / north outgoing
    lyW = CY - LW // 2      # 323 — west incoming  / east outgoing
    lyE = CY + LW // 2      # 397 — east incoming  / west outgoing

    table = {
        # ── North approach (heading DOWN) ────────────────────────────
        # Left turn → exit East: ends on top half (y=lyW, heading right)
        ("north", "east"): dict(
            wp=(lxN, lyW - r),  arc_c=(lxN + r, lyW - r),
            a_s=pi,   a_e=pi / 2,  exit=(1, 0)),
        # Right turn → exit West: ends on bottom half (y=lyE, heading left)
        ("north", "west"): dict(
            wp=(lxN, CY),  arc_c=(lxN - r, CY),
            a_s=0,    a_e=pi / 2,  exit=(-1, 0)),

        # ── South approach (heading UP) ─────────────────────────────
        # Right turn → exit East: ends on top half (y=lyW, heading right)
        ("south", "east"): dict(
            wp=(lxS, CY),  arc_c=(lxS + r, CY),
            a_s=pi,   a_e=-pi / 2,  exit=(1, 0)),
        # Left turn → exit West: ends on bottom half (y=lyE, heading left)
        ("south", "west"): dict(
            wp=(lxS, lyE + r),  arc_c=(lxS - r, lyE + r),
            a_s=0,    a_e=-pi / 2,  exit=(-1, 0)),

        # ── East approach (heading LEFT) ────────────────────────────
        # Left turn → exit South: ends on left half (x=lxN, heading down)
        ("east", "south"): dict(
            wp=(CX, lyE),  arc_c=(CX, lyE + r),
            a_s=-pi / 2,  a_e=pi,  exit=(0, 1)),
        # Right turn → exit North: ends on right half (x=lxS, heading up)
        ("east", "north"): dict(
            wp=(lxS + r, lyE),  arc_c=(lxS + r, lyE - r),
            a_s=pi / 2,   a_e=-pi,  exit=(0, -1)),

        # ── West approach (heading RIGHT) ───────────────────────────
        # Left turn → exit North: ends on right half (x=lxS, heading up)
        ("west", "north"): dict(
            wp=(CX, lyW),  arc_c=(CX, lyW - r),
            a_s=pi / 2,   a_e=0,  exit=(0, -1)),
        # Right turn → exit South: ends on left half (x=lxN, heading down)
        ("west", "south"): dict(
            wp=(lxN - r, lyW),  arc_c=(lxN - r, lyW + r),
            a_s=-pi / 2,  a_e=0,  exit=(0, 1)),

        # ── 180° U-turns (through intersection centre) ─────────────
        ("north", "north"): dict(
            wp=(lxN, CY),  arc_c=(CX, CY),
            a_s=pi,   a_e=0,   exit=(0, -1)),
        ("south", "south"): dict(
            wp=(lxS, CY),  arc_c=(CX, CY),
            a_s=0,    a_e=-pi, exit=(0, 1)),
        ("east", "east"): dict(
            wp=(CX, lyE),  arc_c=(CX, CY),
            a_s=pi / 2,  a_e=3 * pi / 2,  exit=(1, 0)),
        ("west", "west"): dict(
            wp=(CX, lyW),  arc_c=(CX, CY),
            a_s=-pi / 2,  a_e=pi / 2,  exit=(-1, 0)),
    }

    key = (approach, exit_to)
    if key not in table:
        return  # fallback: vehicle goes straight (shouldn't happen)
    ...
    # For U-turns the span is exactly ±π — preserve the sign
    if abs(abs(span) - pi) < 0.001 and key[0] == key[1]:
        span = a_end - a_start  # use raw span for 180° arcs
...
class Vehicle:
    ...
    def __init__(self, vtype: str, direction: str):
        ...
        # ── Intersection turn choice ─────────────────────────
        if vtype == "ambulance":
            self.turn_choice = "straight"       # ambulances never turn
        else:
            choices = list(TURN_WEIGHTS.keys())
            weights = list(TURN_WEIGHTS.values())
            self.turn_choice = random.choices(choices, weights=weights, k=1)[0]
    ...
    def update(self, dt: float, green: bool, vehicles_ahead: "list[Vehicle]"):
        ...
        if not self.passed and not self.in_junction and self.current_speed < 1.0:
            self._total_wait_time += dt

        # ── Intersection turn trigger ─────────────────────────────────
        dist = self.dist_to_stop()
        if (self.turn_choice != "straight"
                and not self.is_diverted
                and not self.in_junction
                and not self.passed
                and not self.is_crashed
                and dist <= 0
                and green):
            exit_dir = TURN_EXIT_MAP.get((self.direction, self.turn_choice))
            if exit_dir:
                _setup_turn_path(self, self.direction, exit_dir)
                self.is_diverted   = True
                self._divert_phase = 0
                self._divert_arc_t = 0.0
                self.in_junction   = True   # NOT passed — stay visible to followers
                return
    ...
    def draw(self, surface: pygame.Surface):
        ...
        # ── Ground shadow ────────────────────────────────────
        if not self.is_diverted:
            shadow = _get_shadow(self.vtype, self.direction)
            sr = shadow.get_rect(center=(ix + SHADOW_OFFSET_X, iy + SHADOW_OFFSET_Y))
        ...
        if self.is_diverted:
            ...
            rect     = vsurface.get_rect(center=(ix, iy))
            surface.blit(vsurface, rect)

            # ── Headlight beam (not for bikes) ───────────────
            if self.vtype != "bike" and not self.is_crashed:
                hl_surf, hl_offset = _get_headlight(self.direction)
                nx, ny = int(self.nose_x), int(self.nose_y)
                surface.blit(hl_surf, (nx + hl_offset[0], ny + hl_offset[1]))

            # ── Brake glow (when braking / stopped) ──────────
            if (not self.is_crashed
                    and not self.passed
                    and self.current_speed < self.speed * 0.3):
                bg = _get_brake_glow()
                r = BRAKE_GLOW_RADIUS
                tx = int(self.x - self.dx * self.body_len / 2)
                ty = int(self.y - self.dy * self.body_len / 2)
                surface.blit(bg, (tx - r, ty - r))
